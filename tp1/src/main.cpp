...
struct App : public OpenGLApplication
{
    ...
    void generateNgon(std::vector<VertexData>* vertices, std::vector<GLuint>* elements, unsigned int side)
    {
    ...
    void sceneShape()
    {
        ...
        glBindVertexArray(vao_);
        //glBindVertexArray(vao_);
        //glPolygonMode(GL_FRONT_AND_BACK, GL_LINE); //poygon mode?
...
private:
    ...
    static constexpr unsigned int MAX_VERTEX_COUNT = MAX_N_SIDES + 1;
    static constexpr unsigned int MAX_INDEX_COUNT = (MAX_N_SIDES - 2) * 3;
